...
// Gryphon Flying: the day clock runs through the sunrise opening and then
// holds at mid-morning (day clock: sunrise 0.125, noon 0.5), so there is no night and the light
// stays readable for the kids. `updateAtmosphere` applies the hold.
const DAY_HOLD = 0.33;
...
// Gryphon Flying: every visit is a fresh world that opens on the sunrise.
// The engine still writes its resume record; it is never read back.
const storedFlight = null;
...
// The player's wings are the gryphon's; the flock's are the kit's.
const animateWings = (g, dt, flapping, speedMul = 1) =>
  g === bird ? animateGryphon(dt, flapping) : animateBird(g, dt, flapping, speedMul);
const bird = makeBird(birdKindId);
scene.add(bird);
// ---------------------------------------------------------------------------
// Gryphon Flying: the player flies as the gryphon. The kind's data still drives
// the flight (bob, floor, look) and the flock stays the kit's birds, but the
// player's meshes are the procedural gryphon in `characters/gryphon.js`, lit by
// the engine's own material so it takes the fog, the color grade and the soft
...
gryphon.root.scale.setScalar(GRYPHON_SPAN / gryphon.dimensions.wingspan);
bird.add(gryphon.root);
// Pose weights blend toward the flight: wings beat while climbing, sweep back
...
function steerByKeys(dt) {
  ...
  const aim = (held.has('ArrowUp') ? 1 : 0) - (held.has('ArrowDown') ? 1 : 0);
  if (turn) state.steer += turn * KEY_RATE.turn * dt;
  if (aim) aimBy(aim * KEY_RATE.aim * dt);
}
...
const CAMERA = {
  dist: 21, // Gryphon Flying: a little further back than upstream's 17, for the wider wings
...
function updateAtmosphere(dt) {
  ...
  if (dayPhase < DAY_HOLD) dayPhase = Math.min(DAY_HOLD, dayPhase + (dt * dayRate) / DAY_SECONDS);
